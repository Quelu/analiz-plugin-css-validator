/**
 * CSS validator module for Analiz
 * @type {Object}
 */
module.exports = {
  ...
  run: function ( files, options, callback ) {
    ...
    files.forEach(function( file ) {
      fs.readFile( file, { encoding: 'utf-8' }, function ( err, content ) {
        ...
        validateCss( options, function ( analizeError, data ) {
          if ( analizeError ) {
            callback( {
              title: 'ERROR : ' + module.exports.config.name[ options.language ],
              content: analizeError
            }, null );
          }
          var errors = [];
